...
const ext = {
  ...
  init(app) {
    ...
    if (auth_token) {
      ...
      localStorage.setItem("comflow_env", "cloud");
    }
    ...
    window.addEventListener("message", async (event) => {
      try {
        ...
        if (message.type === "graph_load") {
          ...
          console.log("received: ", comfyUIWorkflow);
...
async function deployWorkflow() {
  ...
  let deployMeta = graph.findNodesByType("Comflow");
  ...
  if (deployMeta.length == 0) {
    ...
    var node = LiteGraph.createNode("Comflow");
    node.configure({
      widgets_values: [text],
    });
    node.pos = [0, 0];
  ...
  if (includeDeps) {
    ...
    deps = await generateDependencyGraph({
      ...
      computeFileHash: async (file) => {
        ...
        const hash = await fetch(
          `/comflow/get-file-hash?file_path=${encodeURIComponent(file)}`,
        ).then((x) => x.json());
        loadingDialog.showLoading("Generating hash", file);
        console.log(hash);
      ...
      handleFileUpload: async (file, hash, prevhash) => {
        ...
        try {
          const { download_url } = await fetch(`/comflow/upload-file`, {
            method: "POST",
            body: JSON.stringify({
              file_path: file,
              token: apiKey,
    ...
    if (
      !deps.custom_nodes["https://github.com/comfypod/ComfyUI-Comflow"] &&
      !deps.custom_nodes["https://github.com/comfypod/ComfyUI-Comflow.git"]
    )
      deps.custom_nodes["https://github.com/comfypod/ComfyUI-Comflow"] = {
        url: "https://github.com/comfypod/ComfyUI-Comflow",
...
function getData(environment) {
  const deployOption =
    environment || localStorage.getItem("comflow_env") || "cloud";
  const data = localStorage.getItem("comflow_env_data_" + deployOption);
  if (!data) {
    if (deployOption === "cloud")
      return {
        endpoint: "https://comfypod.net",
        apiKey: "",
      };
    else
      return {
...
function saveData(data) {
  localStorage.setItem(
    "comflow_env_data_" + data.environment,
    JSON.stringify(data),
  );
}
...
export class ConfigDialog extends ComfyDialog {
  ...
  save(api_key, displayName) {
    ...
    localStorage.setItem("comflow_env", deployOption);
  ...
  show() {
    ...
    <h3 style="margin: 0px;">Comflow Config</h3>
    <label style="color: white; width: 100%;">
      ...
      <div style="color: white;">
        ...
        <button id="loginButton" style="margin-top: 8px; width: 100%; height:40px; box-sizing: border-box; padding: 0px 6px;">
          ${
            data.apiKey ? "Re-login with ComfyPod" : "Login with ComfyPod"
          }
        </button>
      </div>
      </div>
    ...
    deployOption.addEventListener("change", function () {
      ...
      localStorage.setItem("comflow_env", selectedOption);
